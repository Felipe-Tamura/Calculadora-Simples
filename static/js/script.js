...
function criarTeclas() {
    ...
    teclasAcao.forEach(tecla => {
        ...
        simbolo.onclick = () => { getSymbol(simbolo.value) } //Ação do botão
    ...
    teclasLimpar.forEach(tecla => {
        ...
        simbolo.textContent = tecla //Conteudo do botão
        simbolo.value = tecla //Valor do botão
        simbolo.onclick = () => { limpar(simbolo.value) } //Ação do botão
    ...
    teclaCalcular.onclick = () => { calcular() }
...
function limpar(value){
    ...
    if (value == "C"){// Limpando as duas telas
        telaConsole.innerText = ""
        telaOperacao.innerText = ""
    }else if (value == "CE"){// Limpando apenas o console
        telaConsole.innerText = ""
...
function calcular(){
    ...
    try {
        ...
        if (resultado == "Infinity"){// Verificando se há divisão por zero
...
function generateTable(){
    ...
    if(operacoes.length > 0){// Adicionando valores
